 ...
 * 默认 "行" 间距
 */
static CGFloat const JKDefaultRowMargin    = 0;
/**
 * 默认 "列" 间距
 */
 ...
 * 默认 "边" 间距
 */
static UIEdgeInsets const JKDefaultInsets  = {0,0,0,0};

@interface MeiRongZuJiCollectionLayout()
...
@implementation MeiRongZuJiCollectionLayout
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"self.higtharrar.count = %ld",self.higtharrar.count);
    CGFloat height= [[self.higtharrar objectAtIndex:indexPath.item] floatValue];//每个单元格文字内容的整个高度,160+20图片和姓名高度和
    NSLog(@"ViewLayou的高度:%f",height);
